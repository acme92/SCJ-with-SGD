...
run, k, temp, AorB, alpha = argv[1], argv[2], argv[3], argv[4], argv[5]

#Main program
output_folder = os.path.join("output","with_D","Run_"+run+"_"+k+"_"+temp+AorB+"_"+alpha)
#output_folder = os.path.join("test_output","Run_"+run+"_"+k+"_"+temp,"ILP_output"+"_"+alpha)
...
cont_file = os.path.join("formatted_input","with_D","Run_"+run+"_"+k+"_"+temp+AorB,"GeneOrders.txt")
#cont_file = os.path.join("test_output","Run_"+run+"_"+k+"_"+temp,"GeneOrders.txt")
cont_dict = {}
cont_dict_by_idx = {}
...
ortho_file = os.path.join("formatted_input","with_D","Run_"+run+"_"+k+"_"+temp+AorB,"Orthology.txt")
#ortho_file = os.path.join("test_output","Run_"+run+"_"+k+"_"+temp,"Orthology.txt")
tree_dict = {}
rev_tree_dict = {}
ortho_reln = {}
rev_ortho_reln = {}
...
for species in rev_tree_dict:
	...
	if species not in tree_dict:
		if species in cont_dict_by_idx:
			ext_adj_set[species] = list(SPP_preprocessing.get_adj_set(cont_dict_by_idx[species]))
...
for species in tree_dict:
	...
	if species in cont_dict_by_idx:
		...
		anc_adj_set[species] = list(SPP_preprocessing.get_adj_set(cont_dict_by_idx[species]))
...
adjwt_file = os.path.join("formatted_input","with_D","Run_"+run+"_"+k+"_"+temp+AorB,"Adjacencies.txt")
#adjwt_file = os.path.join("test_output","Run_"+run+"_"+k+"_"+temp,"Adjacencies.txt")
lambda_dict = {}
alpha_dict = {}
n_observed = {}
...
while circular == 1:
	...
	circular, circ_chr, soln_chr_dict = SPP_rmcircular.check_if_circular(reached, soln_chr_dict, circ_chr, c, i, soln_ext_dict, soln_adj_dict, pva_dict, logfile_3, logfile_4)
