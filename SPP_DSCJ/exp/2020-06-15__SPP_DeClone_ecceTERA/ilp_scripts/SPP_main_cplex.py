...
import SPP_prep_cplex
import SPP_rmcir_cplex
import SPP_eval_cplex

full_start = time.time()

home_folder = "../../../"
#exp_folder = "exp/2019-07-15_SPP_test/"
exp_folder = "exp/2020-06-15__SPP_DeClone_ecceTERA/"
...
cont_dict = {}
cont_dict_by_idx = {}
cont_dict, cont_dict_by_idx = SPP_prep_cplex.gene_cont(cont_file, cont_dict, cont_dict_by_idx)
...
#ortho_file = os.path.join(home_folder, exp_folder,"formatted_input",mode,"Run_"+run+"_T0_"+temp+rep,"ecceTERA_ortho.txt")
ortho_file = os.path.join(home_folder, exp_folder,"formatted_input",mode,"Run_"+run+"_T0_"+temp+rep,"ecceTERA_ortho.txt")
...
tree_dict, rev_tree_dict, ortho_reln, rev_ortho_reln \
	= SPP_prep_cplex.ortho(ortho_file, tree_dict, rev_tree_dict, ortho_reln, rev_ortho_reln, cont_dict, logfile_1)
...
nd_dict = {}
ng_dict = {}
gene_losses, nd_dict = SPP_prep_cplex.get_gene_losses(gene_losses, nd_dict, ortho_reln, cont_dict)
gene_gains, ng_dict = SPP_prep_cplex.get_gene_gains(gene_gains, ng_dict, rev_ortho_reln, cont_dict)
...
for species in rev_tree_dict:
	...
	if species not in tree_dict:
		if species in cont_dict_by_idx:
			ext_adj_set[species] = list(SPP_prep_cplex.get_ext_adj_set(cont_dict_by_idx[species]))
...
for species in tree_dict:
	...
	if species in cont_dict_by_idx:
		...
		anc_adj_set[species] = list(SPP_prep_cplex.get_anc_adj_set(cont_dict_by_idx[species]))
...
for species in rev_tree_dict:
	...
	pva_dict, wva_dict, C_dict = SPP_prep_cplex.create_dicts(species, parent, pva_dict, wva_dict, C_dict)
for species in tree_dict:
...
m, pva_dict, C_dict, wva_dict, n_observed, alpha_dict, lambda_dict \
	= SPP_prep_cplex.create_vars(adjwt_file, logfile_2, m, pva_dict, C_dict, wva_dict, n_observed, alpha_dict, lambda_dict, \
...
for species in pva_dict:
	for adj in pva_dict[species]:
		expr.add(alpha*wva_dict[species][adj])
		expr.add_term(pva_dict[species][adj], -alpha*wva_dict[species][adj])
		
	parent = rev_tree_dict[species] if species in rev_tree_dict else None
	if parent:
		for adj in C_dict[(parent, species)]:
			expr.add_term(C_dict[(parent, species)][adj], 1-alpha)
		for adj in C_dict[(species, parent)]:
			expr.add_term(C_dict[(species, parent)][adj], 1-alpha)

		expr.add(2*(1-alpha)*nd_dict[(parent, species)])

		expr.add((1-alpha)*len(gene_losses[(parent, species)]))		

		expr.add(2*(1-alpha)*ng_dict[(species, parent)])

		for gene in n_observed[(parent, species)]:
			expr.add_term(n_observed[(parent, species)][gene], -2*(1-alpha))
		
		for gene in alpha_dict[(parent, species)]:
			expr.add_term(alpha_dict[(parent, species)][gene], 2*(1-alpha))
...
def C_expr(adj, species, parent, ortho_reln, cont_dict, i, premul):
	...
	for member in fam:
		if member in pva_dict[species]:
			expr.add_term(pva_dict[species][member], -i)
		elif member[::-1] in pva_dict[species]:
			expr.add_term(pva_dict[species][member[::-1]], -i)
	if adj in pva_dict[parent]:	
		expr.add_term(pva_dict[parent][adj], i*premul)
	return expr
...
for species in ext_adj_set:	
	for adj in ext_adj_set[species]:
		if adj in pva_dict[species]:
			m.add_constraint(pva_dict[species][adj] == 1, "extant")
			nconstr += 1
		elif adj[::-1]:
			m.add_constraint(pva_dict[species][adj[::-1]] == 1, "extant")
			nconstr += 1
...
for species in rev_tree_dict:																			
	...
	if parent:																							
		for adj in C_dict[(parent,species)]:															
			m.add_constraint(C_dict[(parent,species)][adj] >= 0, "non-negativity"+parent+"to"+species)
			premul = assign_premul(adj, parent, species, gene_losses)
			expr = C_expr(adj, species, parent, ortho_reln, cont_dict, 1, premul)
			m.add_constraint(C_dict[(parent,species)][adj] >= expr, parent+"to"+species)
			nconstr += 1

		for adj in C_dict[(species,parent)]:
			m.add_constraint(C_dict[(species,parent)][adj] >= 0, "non-negativity"+species+"to"+parent)
			premul = assign_premul(adj, parent, species, gene_losses)
			expr = C_expr(adj, species, parent, ortho_reln, cont_dict, -1, premul)
			m.add_constraint(C_dict[(species,parent)][adj] >= expr, species+"to"+parent)
			nconstr += 1
...
for ext in extr_dict:		#For every extremity, the expression ext_expr
	ext_expr = m.linear_expr()	#Sums the pva variables for every adjacency containing a particular extremity
	species = extr_dict[ext]['Species']
	for adj in extr_dict[ext]['Adj_List']:
		ext_expr.add_term(pva_dict[species][adj], 1)

	m.add_constraint(ext_expr <= 1, "consistency")
	nconstr += 1		
	...
	if parent in cont_dict:
		for gene in cont_dict[parent]:
				...
				if branch in observed_dups:
					if gene in observed_dups[branch]:
						for adj in observed_dups[branch][gene]:
							expr.add_term(pva_dict[child][adj], 1)

		
			if copy_no > 0:	
				expr_ubd = m.linear_expr()
				expr1 = expr/copy_no
				m.add_constraint(lambda_dict[branch][gene]['floor'] >= expr1 - 0.99, "floor_lower_bound")
				m.add_constraint(lambda_dict[branch][gene]['floor'] <= expr1, "floor_upper_bound")
				m.add_constraint(lambda_dict[branch][gene]['ceil'] >= expr1, "ceil_upper_bound")
				m.add_constraint(lambda_dict[branch][gene]['ceil'] <= expr1 + 0.99, "ceil_upper_bound")
			else:
				m.add_constraint(lambda_dict[branch][gene]['floor'] == 0, "no_g_chr" )
				m.add_constraint(lambda_dict[branch][gene]['ceil'] == 0, "no_g_chr" )
			...
			if p_adj in pva_dict[parent]:	
				m.add_constraint(alpha_dict[branch][gene] <= pva_dict[parent][p_adj], "alpha"+str(gene))
			elif p_adj[::-1] in pva_dict[parent]:
				m.add_constraint(alpha_dict[branch][gene] <= pva_dict[parent][p_adj[::-1]], "alpha"+str(gene))
			else:
				m.add_constraint(alpha_dict[branch][gene] <= 0, "alpha"+str(gene))
			m.add_constraint(alpha_dict[branch][gene] <= lambda_dict[branch][gene]['ceil'] - lambda_dict[branch][gene]['floor'], "alpha"+str(gene)) 

			expr.add_term(lambda_dict[branch][gene]['floor'],-1)
			m.add_constraint(n_observed[branch][gene] == expr, "observed_dups")
	gains_expr = m.linear_expr()
	gains_expr.add(0)
	if branch in observed_gains:
		for adj in observed_gains[branch]:
			gains_expr.add_term(pva_dict[child][adj], 1)
		m.add_constraint(n_observed[branch]['gains'] == gains_expr, "observed_gains")
...
while circular == 1:
	...
	soln_ext_dict = {}
	soln_adj_dict = {}
	soln_ext_dict, soln_adj_dict = SPP_rmcir_cplex.get_adj_soln(soln_ext_dict, soln_adj_dict, pva_dict, soln)		
	...
	circular, circ_chr, soln_chr_dict = SPP_rmcir_cplex.check_if_circular(reached, soln_chr_dict, circ_chr, c, i, soln_ext_dict, soln_adj_dict, pva_dict, logfile_3)
		...
		if i <= 50:
			...
			for adj in chosen_chr:
				if adj in pva_dict[species]:
					...
					expr.add_term(pva_dict[species][adj], 1)
				elif adj[::-1] in pva_dict[species]:
					logfile_3.write(str(species)+": "+ str(adj[::-1])+"\n")
					expr.add_term(pva_dict[species][adj[::-1]], 1)
			logfile_3.write("\n\n")		
			m.add_constraint(expr <= len(chosen_chr) - 1, "circular")
...
gained_adjs_total = 0
final_adjs = {}
final_adjs, gained_adjs, gained_adjs_total = SPP_eval_cplex.adj_gains(ext_adj_set, initial_adjs, final_adjs, gained_adjs, gained_adjs_total, pva_dict, final_log, soln)
#print(species, initial_adjs[species], gained_adjs[species])


final_scaffolds = {}
anc_scaffolds = {}
final_scaffolds, anc_scaffolds = SPP_eval_cplex.count_scaffolds(ext_adj_set, tree_dict, initial_scaffolds, final_scaffolds, anc_scaffolds, soln_chr_dict, final_log)

SPP_eval_cplex.selected_adjacencies(pva_dict, wva_dict, final_log, soln)


SPP_eval_cplex.compute_stats(pva_dict, anc_adj_set, ext_adj_set, actual_anc_adjs_total, final_log, soln)


cuts_and_joins = open(os.path.join(output_folder,'Cuts_and_joins.log'),"w")
	
SPP_eval_cplex.get_distance_details(pva_dict, rev_tree_dict, C_dict, ng_dict, nd_dict, n_observed, gene_gains, gene_losses, final_log, cuts_and_joins, soln)
