...
def adj_gains(ext_adj_set, initial_adjs, final_adjs, gained_adjs, gained_adjs_total, pva_dict, final_log, soln):
	initial_adjs_total = 0
	for species in ext_adj_set:
		final_adjs[species] = 0
		for adj in pva_dict[species]:
			g1,e1,g2,e2 = get_adj_details(adj)
			if soln.get_value("p_"+str(species)+"_"+str(g1)+str(e1)+str(g2)+str(e2)) > 0:
...
def selected_adjacencies(pva_dict, wva_dict, final_log, soln):
...
def compute_stats(pva_dict, anc_adj_set, ext_adj_set, actual_anc_adjs_total, final_log, soln):
...
def list_cuts_and_joins(parent, species, distance, C_dict, gene_gains, gene_losses, cuts_and_joins, soln):
	cuts_and_joins.write("\nFrom "+parent+ " to "+ species+"\n")
	cuts_and_joins.write("Cuts:\n")
	for adj in C_dict[(parent, species)]:
		g1,e1,g2,e2 = get_adj_details(adj)		
		distance[(parent, species)]['Cuts'] += soln.get_value('C_'+str(species)+str(parent)+'_'+str(g1)+str(e1)+str(g2)+str(e2))
		distance[(parent, species)]['SCJTDFD'] += soln.get_value('C_'+str(parent)+str(species)+'_'+str(g1)+str(e1)+str(g2)+str(e2))

				
		if soln.get_value('C_'+str(parent)+str(species)+'_'+str(g1)+str(e1)+str(g2)+str(e2)) == 1:
		...
		if gene1 in gene_losses[(parent, species)] or gene2 in gene_losses[(parent, species)]:
			distance[(parent, species)]['Cuts_by_Losses'] += soln.get_value('C_'+str(parent)+str(species)+'_'+str(g1)+str(e1)+str(g2)+str(e2))

	cuts_and_joins.write("\nJoins:\n")		
	for adj in C_dict[(species, parent)]:
		distance[(parent, species)]['Joins'] += soln.get_value('C_'+str(species)+str(parent)+'_'+str(g1)+str(e1)+str(g2)+str(e2))
		distance[(parent, species)]['SCJTDFD'] += soln.get_value('C_'+str(species)+str(parent)+'_'+str(g1)+str(e1)+str(g2)+str(e2))

				
		if soln.get_value('C_'+str(species)+str(parent)+'_'+str(g1)+str(e1)+str(g2)+str(e2)) == 1:
			cuts_and_joins.write(str(adj)+"\n")			

		gene1, gene2 = adj[0][0], adj[1][0]
		if gene1 in gene_gains[(species, parent)] or gene2 in gene_gains[(species, parent)]:
			distance[(parent, species)]['Joins_by_Gains'] += soln.get_value('C_'+str(species)+str(parent)+'_'+str(g1)+str(e1)+str(g2)+str(e2))
	cuts_and_joins.write("\n\n")

	return distance	

def get_distance_details(pva_dict, rev_tree_dict, C_dict, ng_dict, nd_dict, n_observed, gene_gains, gene_losses, final_log, cuts_and_joins, soln):	
	...
	for species in pva_dict:
		...
		if parent:
			distance = list_cuts_and_joins(parent, species, distance, C_dict, gene_gains, gene_losses, cuts_and_joins, soln)
			...
			for gene in n_observed[(parent, species)]:
				distance[(parent, species)]['Observed_Dups'] += soln.get_value('n_observed_dups'+str(parent)+str(species)+'_'+str(gene))
				distance[(parent, species)]['SCJTDFD'] += -2*soln.get_value('n_observed_dups'+str(parent)+str(species)+'_'+str(gene))
